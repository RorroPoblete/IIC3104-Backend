...
export class NormaMinsalExcelParser {
  ...
  static async parseFile(filePath: string): Promise<ExcelParseResult> {
    ...
    try {
      ...
      headers = (firstRow as string[]).map((header) =>
        NormaMinsalExcelParser.normalizeHeader(String(header)),
      );
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if (i % 1000 === 0) {
          logger.info(`Procesadas ${i - 1} filas de Norma Minsal`);
        }
      }

      logger.info(`Parseo de Excel de Norma Minsal completado. Total filas: ${rows.length}`);
      ...
      logger.error('Error al parsear Excel de Norma Minsal', error);
